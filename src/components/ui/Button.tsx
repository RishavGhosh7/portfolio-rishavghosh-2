  ...
  'inline-flex cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        cool: 'bg-linear-to-t from-primary to-primary/85 text-primary-foreground shadow-md shadow-primary/20 ring-1 ring-inset ring-white/25 transition-[filter] duration-200 hover:brightness-110 active:brightness-90 dark:inset-shadow-2xs dark:inset-shadow-white/10 dark:border-x-0 dark:border-t-0 dark:border-primary/50 dark:text-primary-foreground dark:ring-white/5 border border-b-2 border-zinc-950/40',
...
const liquidButtonVariants = cva(
  "inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium outline-none transition-[color,box-shadow] transition-colors disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
...
const LiquidButton = React.forwardRef<HTMLButtonElement, LiquidButtonProps>(
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    const content = (buttonContent: React.ReactNode) => (
      <>
        <div
          className="absolute left-0 top-0 z-0 h-full w-full overflow-hidden rounded-full opacity-55 shadow-[0_0_6px_rgba(0,0,0,0.03),0_2px_6px_rgba(0,0,0,0.08),inset_3px_3px_0.5px_-3px_rgba(0,0,0,0.9),inset_-3px_-3px_0.5px_-3px_rgba(0,0,0,0.85),inset_1px_1px_1px_-0.5px_rgba(0,0,0,0.6),inset_-1px_-1px_1px_-0.5px_rgba(0,0,0,0.6),inset_0_0_6px_6px_rgba(0,0,0,0.12),inset_0_0_2px_2px_rgba(0,0,0,0.06),0_0_12px_rgba(255,255,255,0.15)] transition-all dark:shadow-[0_0_8px_rgba(0,0,0,0.03),0_2px_6px_rgba(0,0,0,0.08),inset_3px_3px_0.5px_-3.5px_rgba(255,255,255,0.09),inset_-3px_-3px_0.5px_-3.5px_rgba(255,255,255,0.85),inset_1px_1px_1px_-0.5px_rgba(255,255,255,0.6),inset_-1px_-1px_1px_-0.5px_rgba(255,255,255,0.6),inset_0_0_6px_6px_rgba(255,255,255,0.12),inset_0_0_2px_2px_rgba(255,255,255,0.06),0_0_12px_rgba(0,0,0,0.15)]"
          style={{ backdropFilter: `url("#${filterId}")` }}
        />
        <div className="pointer-events-none z-10">{buttonContent}</div>
...
const MetalButton = React.forwardRef<HTMLButtonElement, MetalButtonProps>(
  (
    ...
    const handleTouchCancel: React.TouchEventHandler<HTMLButtonElement> = (event) => {
      setIsPressed(false)
      onTouchCancel?.(event)
    }

    return (
      <div className={variants.wrapper as string} style={variants.wrapperStyle as React.CSSProperties}>
        <div className={variants.inner as string} style={variants.innerStyle as React.CSSProperties} />
        {asChild ? (
          <Slot
            ref={ref}
            className={cn(variants.button as string, className)}
...
// The CVA variant exports intentionally mirror the standard shadcn button API.
