...
function FullscreenShader() {
  ...
  useFrame(({ clock }) => {
    ...
    ;(gl as THREE.WebGLRenderer).getDrawingBufferSize(bufferSizeRef.current)
    shaderRef.current.uniforms.uResolution.value.set(
      bufferSizeRef.current.x,
      bufferSizeRef.current.y,
      1,
    )
  })
...
export function OfflinePage() {
  return (
    <section className={cn('offline-page')} aria-label="No internet connection">
